...
class SQL(object):
    ...
    router="SELECT routers.tenant_id,routers.id as resource_id,routers.name as resource_name,routers.status,ports.create_at as created_at,'10' as bandwidth \
    FROM neutron.routers LEFT JOIN neutron.ports ON routers.gw_port_id=ports.id WHERE create_at <= DATE_FORMAT(DATE_ADD(CURRENT_TIMESTAMP,INTERVAL -1 HOUR),'%Y-%m-%d %H:%i')"
