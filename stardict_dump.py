...
# -*- coding: utf-8 -*-
#  词典来自: 
# /tmp3/portage/app-dicts/stardict-xdict-en-zh-gb
import struct
import types
import gzip
...
def insert_to_mongo(col, word,chinese):
...
def read_dict_info(word):
    """
    """

    ifo_file="/tmp3/portage/app-dicts/stardict-xdict-en-zh-gb/stardict-xdict-ec-gb-2.4.2/xdict-ec-gb.ifo";
    idx_file="/tmp3/portage/app-dicts/stardict-xdict-en-zh-gb/stardict-xdict-ec-gb-2.4.2/xdict-ec-gb.idx";
    dict_file="/tmp3/portage/app-dicts/stardict-xdict-en-zh-gb/stardict-xdict-ec-gb-2.4.2/xdict-ec-gb.dict.dz";
    ...
    if  isinstance(word,str):
        chs= dict_reader.get_dict_by_word(word)
        try:
            print chs[0]['m']
        ...
        for each in  word:
            print "#===========%s======="%each;
            chs= dict_reader.get_dict_by_word(each);
            chinese=chs[0]['m'];
            print chinese;

            #insert_to_mongo(col, each, chinese);
...
def check():
    if not  os.path.isdir("/tmp3/portage/app-dicts/stardict-xdict-en-zh-gb/stardict-xdict-ec-gb-2.4.2/"):
        print(" cd  /tmp3/portage/app-dicts/stardict-xdict-en-zh-gb/  &&  perl /root/bin/gentoo_downloader.pl ");
        os.system(" cd  /tmp3/portage/app-dicts/stardict-xdict-en-zh-gb/  &&  perl /root/bin/gentoo_downloader.pl ");
        print " you should  extract it ";
        sys.exit(-1);
...
if __name__=="__main__":
    ...
    if len(sys.argv) ==1:
        ...
        idx_file="/tmp3/portage/app-dicts/stardict-xdict-en-zh-gb/stardict-xdict-ec-gb-2.4.2/xdict-ec-gb.idx";
        word_list=read_idx_file(idx_file);
        read_dict_info(word_list);
        sys.exit(-1);

    elif  len(sys.argv)==2:
        #search_one_word(sys.argv); 
        read_dict_info(sys.argv[1]);
